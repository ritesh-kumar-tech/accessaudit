...
interface SubscriptionsResponse { rows: SubscriptionRow[]; page: number; pageSize: number; total: number; razorpayConnected: boolean; }
  ...
  return (
    <div className="p-6 rounded-2xl bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 space-y-4">
      <div>
        ...
        <p className="text-xs text-slate-500">Mirrors Razorpay -- Razorpay stays authoritative; this table is written only by verified webhook events.</p>
      </div>

      {data && !data.razorpayConnected && (
        <NotConnectedBanner message="Razorpay isn't connected on this deployment yet. This view is real and ready, but will stay empty until billing is wired up." />
      )}
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#080D1A] text-slate-500 font-bold uppercase text-[10px]">
                    ...
                    <th className="py-2.5 px-3">Razorpay Customer</th>
                    <th className="py-2.5 px-3">Razorpay Subscription</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {data.rows.map(s => (
                    <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                      ...
                      <td className="py-2.5 px-3 font-mono text-[11px]">{s.razorpay_customer_id || '—'}</td>
                      <td className="py-2.5 px-3 font-mono text-[11px]">{s.razorpay_subscription_id || '—'}</td>
